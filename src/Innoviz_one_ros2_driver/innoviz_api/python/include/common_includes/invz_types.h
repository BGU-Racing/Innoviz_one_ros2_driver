...
CS_IGNORE	const size_t NUMBER_OF_PC_PLUS_DETECTION_POINT = 238301;
	...
	*/
	enum GrabType: uint64_t
	{
		...
		GRAB_TYPE_OGM_OBJECT_LIST = 0x103000,
		GRAB_TYPE_OGM_LIDAR_DETECTIONS = 0x103002,
		GRAB_TYPE_OGM_GEOMETRIES_LIST = 0x103003
...
#define INVZ4_SUMMATION_MACRO_PIXEL_CHANNEL_COUNT static_cast<int>(INVZ4_MACRO_PIXEL_CHANNEL_COUNT*0.5f)
	...
	*/
	inline void MetaHeader::handle_endianess() {

		vb_invzbuf::endianess_switch(&frameId, sizeof(MetaHeader::frameId));
		vb_invzbuf::endianess_switch(&version, sizeof(MetaHeader::version));
		vb_invzbuf::endianess_switch(&firstObject, sizeof(MetaHeader::firstObject));
		vb_invzbuf::endianess_switch(&lastObject, sizeof(MetaHeader::lastObject));
		vb_invzbuf::endianess_switch(&totalObjects, sizeof(MetaHeader::totalObjects));
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline void StdTimestamp::handle_endianess() {

		vb_invzbuf::endianess_switch(&nanoseconds, sizeof(StdTimestamp::nanoseconds));
		vb_invzbuf::endianess_switch(&seconds, sizeof(StdTimestamp::seconds));
		vb_invzbuf::endianess_switch(&sync_state, sizeof(StdTimestamp::sync_state));
	}
	...
	*/
	...
	enum CalibrationStatus {
		...
		CalibStatus_PausedVehicleDynamics = 3,
		CalibStatus_PausedSAV = 4,
		CalibStatus_PausedGround = 5,
		CalibStatus_PausedCleaning = 6,
		CalibStatus_Available = 7,
		CalibrationStatusEnum_INVALID = 255
	};

	struct OCSensorPose
	{
		float yaw_rad; // current_pose_yaw
		float pitch_rad; // current_pose_pitch
		float roll_rad; // current_pose_roll
	...
	*/
	struct OCOutput
	{
		...
		OCSensorPose oc_pose_valid_values;
		OCSensorPose oc_pose_golden_values;
		CalibrationStatus oc_state;
		float oc_age_minutes; // new
		float oc_distance_meters; // new
		float oc_estimated_accuracy; // new
		uint32_t frame_id;
		uint8_t is_golden_value_valid_bitmap;
		uint8_t is_valid_value_valid_bitmap;  
	};
	...
	*/
	 ...
	 * @description handle fields endianes, converts all fields to little endian.
	 */
	inline  void StndTimestamp::handle_endianess() {
		
		vb_invzbuf::endianess_switch(&fractional_seconds, sizeof(StndTimestamp::fractional_seconds));
		vb_invzbuf::endianess_switch(&seconds, sizeof(StndTimestamp::seconds));
		vb_invzbuf::endianess_switch(&sync_status, sizeof(StndTimestamp::sync_status));
	}
	 ...
	 * @description handle fields endianes, converts all fields to little endian.
	 */
	inline  void CoordinateSystemOrigin::handle_endianess() {

		vb_invzbuf::endianess_switch(&invalid_flags, sizeof(CoordinateSystemOrigin::invalid_flags));
		vb_invzbuf::endianess_switch(&x, sizeof(CoordinateSystemOrigin::x));
		vb_invzbuf::endianess_switch(&x_std_dev, sizeof(CoordinateSystemOrigin::x_std_dev));
		vb_invzbuf::endianess_switch(&y, sizeof(CoordinateSystemOrigin::y));
		vb_invzbuf::endianess_switch(&y_std_dev, sizeof(CoordinateSystemOrigin::y_std_dev));
		vb_invzbuf::endianess_switch(&z, sizeof(CoordinateSystemOrigin::z));
		vb_invzbuf::endianess_switch(&z_std_dev, sizeof(CoordinateSystemOrigin::z_std_dev));
		vb_invzbuf::endianess_switch(&roll, sizeof(CoordinateSystemOrigin::roll));
		vb_invzbuf::endianess_switch(&roll_std_dev, sizeof(CoordinateSystemOrigin::roll_std_dev));
		vb_invzbuf::endianess_switch(&pitch, sizeof(CoordinateSystemOrigin::pitch));
		vb_invzbuf::endianess_switch(&pitch_std_dev, sizeof(CoordinateSystemOrigin::pitch_std_dev));
		vb_invzbuf::endianess_switch(&yaw, sizeof(CoordinateSystemOrigin::yaw));
		vb_invzbuf::endianess_switch(&yaw_std_dev, sizeof(CoordinateSystemOrigin::yaw_std_dev));
	}
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline  void Pose::handle_endianess() {

		vb_invzbuf::endianess_switch(&poseStatus, sizeof(Pose::poseStatus));
		vb_invzbuf::endianess_switch(&poseAge, sizeof(Pose::poseAge));
		vb_invzbuf::endianess_switch(&poseDistance, sizeof(Pose::poseDistance));
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline  void OCOutputSI::handle_endianess() {

		vb_invzbuf::endianess_switch(&eventDataQualifier, sizeof(OCOutputSI::eventDataQualifier));

		timestamp.handle_endianess();

		vb_invzbuf::endianess_switch(&calibrationStatus, sizeof(OCOutputSI::calibrationStatus));

		pose.handle_endianess();

		vb_invzbuf::endianess_switch(&lidarPowerMode, sizeof(OCOutputSI::lidarPowerMode));
	}
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline void DetectionListHeader::handle_endianess() {

		vb_invzbuf::endianess_switch(&event_data_qualifier, sizeof(DetectionListHeader::event_data_qualifier));
		vb_invzbuf::endianess_switch(&extended_qualifier, sizeof(DetectionListHeader::extended_qualifier));

		time_stamp.handle_endianess();
		origin.handle_endianess();
	}	
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline void PCPlusMetaData::handle_endianess() {

		header.handle_endianess();

		vb_invzbuf::endianess_switch(&number_of_detections, sizeof(PCPlusMetaData::number_of_detections));
		vb_invzbuf::endianess_switch(&number_of_detections_low_res_left_origin, sizeof(PCPlusMetaData::number_of_detections_low_res_left_origin));
		vb_invzbuf::endianess_switch(&number_of_detections_low_res_right_origin, sizeof(PCPlusMetaData::number_of_detections_low_res_right_origin));
		vb_invzbuf::endianess_switch(&number_of_detections_high_res_left_origin, sizeof(PCPlusMetaData::number_of_detections_high_res_left_origin));
		vb_invzbuf::endianess_switch(&number_of_detections_high_res_right_origin, sizeof(PCPlusMetaData::number_of_detections_high_res_right_origin));
		
		left_origin_in_sensor_origin.handle_endianess();
		right_origin_in_sensor_origin.handle_endianess();
	}
	...
	* @description handle fields endianes, converts all fields to little endian.
	*/
	inline void PCPlusMetadata48k::handle_endianess() {

		header.handle_endianess();

		vb_invzbuf::endianess_switch(&number_of_detections, sizeof(PCPlusMetadata48k::number_of_detections));
		vb_invzbuf::endianess_switch(&number_of_detections_roi_left_origin, sizeof(PCPlusMetadata48k::number_of_detections_roi_left_origin));
		vb_invzbuf::endianess_switch(&number_of_detections_roi_right_origin, sizeof(PCPlusMetadata48k::number_of_detections_roi_right_origin));
		vb_invzbuf::endianess_switch(&number_of_detections_outer_left_origin, sizeof(PCPlusMetadata48k::number_of_detections_outer_left_origin));

		left_origin_in_sensor_origin.handle_endianess();
		right_origin_in_sensor_origin.handle_endianess();

		vb_invzbuf::endianess_switch(&lidarPowerMode, sizeof(PCPlusMetadata48k::lidarPowerMode));
	}
	...
	struct LidarStatus {
		...
		uint32_t sync_stat_bist_cnt_standby_reason; 
		// (byte 0: sync status, byte 1: bist fail count, byte 2-3: safe state reason)
		uint32_t sg1_sg2_tlr1_tlr2; // (8 SG1, 8 SG2, 8 TLR1, 8 TLR3)
		uint32_t tlr4; // (8 TLR4, 24 reserved)
		uint32_t categories;
		uint32_t reserved;
		uint32_t info_num_ind; // ( 24 information, 8 num of active indications)
		uint32_t error_code;
		float vbat;
	...
	struct SafetyMechnismsIndications {
		uint8_t sg1;
		uint8_t sg2;
		uint8_t tslr1;
		uint8_t tslr3;
	...
	struct TlvValue_WatchDog {
		uint32_t frame_number;
		uint32_t internal_timestamp;
		uint32_t timestamp_sec;
		uint32_t timestamp_micro;
		SafetyMechnismsIndications safetyMechanismIndications;
		uint8_t lidar_mode;
		uint8_t lidar_submode;
		uint16_t lidarStateExtension;
		uint16_t vbat;
		uint16_t reserved;
	};
