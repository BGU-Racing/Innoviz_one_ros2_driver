...
namespace invz {
	...
	public:
		...
		*/
		...
		/**
		* @brief Get device exchange keys
		*/
		virtual Result GetExchangeKeys(std::array<std::array<byte, RSA_CYPHER_SIZE_BYTES>, 3>& value) = 0;
		
		/**
		* @brief Encrypt root key for cyber-security handshake
		*/
		virtual Result EncryptRootKey(const std::string& rootKey, const std::array<byte, RSA_CYPHER_SIZE_BYTES>& publicKey, std::array<byte, RSA_CYPHER_SIZE_BYTES>& value) = 0;
		
		/**
		* @brief Set device root key
		*/
		virtual Result SetRootKey(const std::array<byte, RSA_CYPHER_SIZE_BYTES>& rootKey) = 0;
		
		/**
		* @brief Check if Cyber Security hadnshake is required by OM
		*/
		virtual Result IsCSHandshakeRequired(bool &cs_required) = 0;

		/**
		* @brief Perform complete cyber-security handshake
		*/
		virtual Result CSHandshake(const std::string& rootKey) = 0;
		...
		*/
		...
		/** @brief set all tap activation state to false */
		virtual Result DeactivateAllTaps() = 0;
		...
		*/
		...
		/** @brief Set device MIPI communication state.
		*
		* @return Result OK(0) if response received with no error.
		* @param state The requested state
		*/
		virtual Result SetMipiComState(unsigned char state) = 0;
		...
		*/
		...
		/**
		* @brief Get watch-dog message from device
		*/
		virtual Result GetWatchDog(TlvValue_WatchDog& value) = 0;
		...
		*/
		...
		/* Register for TCP disconnected notification */
		virtual Result RegisterDisconnectedCallback(std::function<void()> callback) = 0;
		/**
		* @brief Unregister callback if was registered, else does nothing.
		*/
