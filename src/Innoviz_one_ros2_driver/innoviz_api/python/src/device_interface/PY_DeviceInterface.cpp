...
#include <iostream>
 ...
 PY_DeviceInterface::PY_DeviceInterface(std::string config_file_name, bool is_connect, int login_level, std::string password, uint32_t log_severity, bool require_data_attr, bool require_cs_handshake)
 {
	 invz::Result result;
	 ConnectionLevel = 0;
	 ...
	 if (is_connect)
	 {
		 ...
		 auto callback = std::bind(&PY_DeviceInterface::diDisconnected, this);

		 DI()->RegisterDisconnectedCallback(callback);
	 }
	 ...
	 // Perform Cyber Security handshake
	 if (require_cs_handshake)
	 {
		 auto rootKey = invz::Generate32ByteKey();
		 result = DI()->CSHandshake(rootKey);
		 CheckResult(result);
	 } 

	 // Initialize frame attributes 
	 if (require_data_attr)
	 {
		 ...
		 do
		 {
			 ...
			 if (result.error_code != ERROR_CODE_OK)
			 {
				 ...
				 {
					 attr_count = 0;
					 std::cout << "Warning: Device meta is missing. Point cloud grabbing won't be available" << std::endl;
					 break;
				 }
 ...
 void PY_DeviceInterface::diDisconnected()
 {
 ...
 PyGrabFrameResult PY_DeviceInterface::GetFrame(std::vector<FrameDataAttributes> frame_types)
 {
	 ...
	 for (auto& dataAttrs : frame_types)
	 {
		 ...
		 if (dataAttrs.known_type == GrabType::GRAB_TYPE_PC_PLUS || dataAttrs.known_type == GrabType::GRAB_TYPE_TRACKED_OBJECTS_SI
			 || dataAttrs.known_type == GRAB_TYPE_DETECTIONS_SI || dataAttrs.known_type == GRAB_TYPE_OC_OUTPUT_SI || dataAttrs.known_type == GrabType::GRAB_TYPE_OGM_OBJECT_LIST
			 || dataAttrs.known_type == GrabType::GRAB_TYPE_OGM_LIDAR_DETECTIONS || dataAttrs.known_type == GrabType::GRAB_TYPE_OGM_GEOMETRIES_LIST)
			 // Swap the byte ordering to 'little-endianess' 
			 dataAttrsBuffer.handle_endianess = 1;
 ...
 void PY_DeviceInterface::DeactivateAllTaps()
 {
	 invz::Result result;

	 result = DI()->DeactivateAllTaps();
	 CheckResult(result);
 }
 ...
 TlvValue_WatchDog PY_DeviceInterface::GetWatchDog()
 {
	 invz::Result result;
	 invz::TlvValue_WatchDog watch_dog{};
	 result = DI()->GetWatchDog(watch_dog);
	 CheckResult(result);
	 return watch_dog;
 }

 std::array<std::array<byte, RSA_CYPHER_SIZE_BYTES>, 3> PY_DeviceInterface::GetExchangeKeys()
 {
	 invz::Result result;
	 std::array<std::array<byte, RSA_CYPHER_SIZE_BYTES>, 3> exchangeKeys{};
	 result = DI()->GetExchangeKeys(exchangeKeys);
	 CheckResult(result);
	 return exchangeKeys;
 }

 std::array<byte, RSA_CYPHER_SIZE_BYTES> PY_DeviceInterface::EncryptRootKey(const std::string & rootKey, const std::array<byte, RSA_CYPHER_SIZE_BYTES>& publicKey)
 {
	 invz::Result result;
	 std::array<byte, RSA_CYPHER_SIZE_BYTES> encryptedRootKey{};
	 result = DI()->EncryptRootKey(rootKey, publicKey, encryptedRootKey);
	 CheckResult(result);
	 return encryptedRootKey;
 }

 void PY_DeviceInterface::SetRootKey(const std::array<byte, RSA_CYPHER_SIZE_BYTES>& rootKey)
 {
	 invz::Result result;
	 result = DI()->SetRootKey(rootKey);
	 CheckResult(result);
 }

 void PY_DeviceInterface::CSHandshake(const std::string& rootKey)
 {
	 invz::Result result;
	 result = DI()->CSHandshake(rootKey);
	 CheckResult(result);
