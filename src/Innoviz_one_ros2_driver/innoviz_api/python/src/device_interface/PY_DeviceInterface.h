...
{
public:
	PY_DeviceInterface(std::string config_file_name, bool is_connect, int login_level, std::string password, uint32_t log_severity, bool require_data_attr, bool require_cs_handshake);
	~PY_DeviceInterface();
	...
	TlvValue_WatchDog GetWatchDog();
	std::array<std::array<byte, RSA_CYPHER_SIZE_BYTES>, 3> GetExchangeKeys();
	std::array<byte, RSA_CYPHER_SIZE_BYTES> EncryptRootKey(const std::string& rootKey, const std::array<byte, RSA_CYPHER_SIZE_BYTES>& publicKey);
	void SetRootKey(const std::array<byte, RSA_CYPHER_SIZE_BYTES>& rootKey);
	void CSHandshake(const std::string& rootKey);
