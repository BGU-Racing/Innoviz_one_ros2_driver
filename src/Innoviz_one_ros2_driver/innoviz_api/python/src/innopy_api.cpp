...
PYBIND11_MODULE(api, m) {
	...
	PYBIND11_NUMPY_DTYPE(invz::LidarStatus, system_mode, timestamp_sec, timestamp_usec, sync_stat_bist_cnt_standby_reason, sg1_sg2_tlr1_tlr2, tlr4, categories, reserved, info_num_ind, error_code, vbat, indications);\
	...
	PYBIND11_NUMPY_DTYPE(invz::ObjectDetection, time_stamp, unique_id, coord_system, ref_point_type, position, dim_and_occlusion,
		...
		PYBIND11_NUMPY_DTYPE(invz::Sensor_Pose_Data, frame_id, timestamps_mili_sec, sp_pitch_deg, sp_roll_deg, sp_z_cm, sp_plane, sp_mat3x3, n_of_inliers, sp_fit_quality_0_to_1, sp_smoothened_yaw, sp_x_cm, sp_y_cm, reserved); \
	...
	PYBIND11_NUMPY_DTYPE(invz::PCPlusDetection, distance, positive_predictive_value, reflectivity, classification, height, angle_azimuth,
			angle_elevation); \
	PYBIND11_NUMPY_DTYPE(invz::OCSensorPose, yaw_rad, pitch_rad, roll_rad, z_above_ground_cm); \
	PYBIND11_NUMPY_DTYPE(invz::OCOutput, is_oc_data_not_valid, oc_pose_status, oc_pose_valid_values, oc_pose_golden_values, oc_state, oc_age_minutes, oc_distance_meters,
		oc_estimated_accuracy, frame_id, is_golden_value_valid_bitmap, is_valid_value_valid_bitmap);\
	...
	PYBIND11_NUMPY_DTYPE(vb_invzbuf::InnerPointLidar, invalid_flags, x, y); \
	PYBIND11_NUMPY_DTYPE(vb_invzbuf::ObjectPodLidar, summary, existence, statusSensors, position, dynamics, shape3d, classification, inner_point); \
	...
	PYBIND11_NUMPY_DTYPE(invz::PCPlusMetadata48k, header, number_of_detections, number_of_detections_roi_left_origin, number_of_detections_roi_right_origin, number_of_detections_outer_left_origin
		, left_origin_in_sensor_origin, right_origin_in_sensor_origin, lidarPowerMode);
	...
	PYBIND11_NUMPY_DTYPE(invz::OCOutputSI, eventDataQualifier, timestamp, calibrationStatus, pose, lidarPowerMode); \
	PYBIND11_NUMPY_DTYPE(invz::FoVRegionLidar, blockageClassification, detectionQualityIndex); \
	PYBIND11_NUMPY_DTYPE(invz::FOVOutputSI, eventDataQualifier, extendedDataQualifier, timestamp, regions, classificationUpdateFlag, bwdStatus, lidarPowerMode); \
	PYBIND11_NUMPY_DTYPE(invz::InterfaceVersionId, major, minor, patch); \
	PYBIND11_NUMPY_DTYPE(invz::SensorOriginPoint, x, y, z); \
	PYBIND11_NUMPY_DTYPE(invz::SensorOrientation, data); \
	PYBIND11_NUMPY_DTYPE(invz::SensorPose, origin, orientation); \
	PYBIND11_NUMPY_DTYPE(invz::SegmentAzimuth, begin, end); \
	PYBIND11_NUMPY_DTYPE(invz::SegmentElevation, begin, end); \
	PYBIND11_NUMPY_DTYPE(invz::DetectionRangeRadialDistance, begin, end); \
	PYBIND11_NUMPY_DTYPE(invz::ReferenceTargetRecognitionCapability, reference_reflectivity, range); \
	PYBIND11_NUMPY_DTYPE(invz::FieldOfViewSegment, azimuth, elevation, blockage, target_recognition_capability); \
	PYBIND11_NUMPY_DTYPE(invz::FieldOfViewSegments, segment); \
	PYBIND11_NUMPY_DTYPE(invz::LidarSensorDetectionsHeader, version_id, num_sensors, sensor_id, timestamp_measurement, cycle_counter, qualifier, vehicle_coordinate_system, sensor_pose, asil, sections); \
	PYBIND11_NUMPY_DTYPE(invz::Position, radial_distance, azimuth, elevation); \
	PYBIND11_NUMPY_DTYPE(invz::LidarDetectionEntity, existence_probability, reflectivity, position); \
	PYBIND11_NUMPY_DTYPE(invz::SensorSpecs, number_of_returns, ordered, scan_data_start_position, scan_data_direction); \
	PYBIND11_NUMPY_DTYPE(invz::OGM_LidarDetectionInterface, num_fov_segments, num_ref_targets, header, detections, sensor_specs); \
	PYBIND11_NUMPY_DTYPE(invz::TimeStamp, high, low); \
	PYBIND11_NUMPY_DTYPE(invz::VersionInfo, major, minor, patch); \
	PYBIND11_NUMPY_DTYPE(invz::Header, version, sensorID, asil, timeStamp, frameID); \
	PYBIND11_NUMPY_DTYPE(invz::DetectionRange, radiusStart, radiusEnd, azimuthStart, azimuthEnd, elevationStart, elevationEnd, confidence, objectType); \
	PYBIND11_NUMPY_DTYPE(invz::DetectionRangeList, numDetectionRanges, detectionRangeList); \
	PYBIND11_NUMPY_DTYPE(invz::SensorPoseObjects, sensorOriginX, sensorOriginXStdDev, sensorOriginY, sensorOriginYStdDev, sensorOriginZ, sensorOriginZStdDev, sensorYaw, sensorYawStdDev, sensorPitch, sensorPitchStdDev, sensorRoll, sensorRollStdDev); \
	PYBIND11_NUMPY_DTYPE(invz::SensorStatus, sensorOrigin, calibration, unavailable, failure, detectionRanges); \
	PYBIND11_NUMPY_DTYPE(invz::Classification, classType, probability); \
	PYBIND11_NUMPY_DTYPE(invz::SensorContribution, timeOfLastMeasuredBySensor, timeOfFirstMeasuredBySensor, measurementCount); \
	PYBIND11_NUMPY_DTYPE(invz::Edge, startPointMeasuredStatus, endPointMeasuredStatus, measurement, stdDev); \
	PYBIND11_NUMPY_DTYPE(invz::Object, objectID, timeOffset, historyFlag, existenceProbability, classification, ambiguity, measStatus, sensorContributions, positionX, positionXStdDev, positionY, positionYStdDev, positionZ, 
						positionZStdDev, heightAboveGround, heightAboveGroundStdDev, yawAngle, yawAngleStdDev, velocityX, velocityXStdDev, velocityY, velocityYStdDev, velocityZ, velocityZStdDev, length, width, height, lengthSide,
						widthSide, correlationPositionXY, correlationPositionYZ, correlationPositionXZ, correlationPositionXYawAngle, correlationPositionYYawAngle, correlationVelocityXY, refPointRatioLength, refPointRatioWidth, refPointRatioHeight); \
	PYBIND11_NUMPY_DTYPE(invz::object_list_OGM, numberOfObj, objectList, linkTimestamp); \
	PYBIND11_NUMPY_DTYPE(invz::OGM_ObjectList, header, sensorStatus, tracked_objects); \
	PYBIND11_NUMPY_DTYPE(invz::GeometryVertex, positionX, positionXStdDev, positionY, positionYStdDev, positionZ, positionZStdDev, heightAboveGround, heightAboveGroundStdDev, height, heightStdDev, width, widthStdDev, correlationXY,
		                 correlationXZ, correlationYZ, existenceProbability, freespaceProbability, classification, objectID); \
	PYBIND11_NUMPY_DTYPE(invz::GeometryContour, geometryID, history, type, flags, lineType, startVertexIndex, numberOfVertices, classification); \
	PYBIND11_NUMPY_DTYPE(invz::GeometryList, numberOfContours, contourList, vertexList); \
	PYBIND11_NUMPY_DTYPE(invz::OGM_GeometryList, header, sensorStatus, geometryList); \
	...
	py::enum_<GrabType>(m, "GrabType")
		...
		.value("GRAB_TYPE_OGM_LIDAR_DETECTIONS", GrabType::GRAB_TYPE_OGM_LIDAR_DETECTIONS)
		.value("GRAB_TYPE_OGM_OBJECT_LIST", GrabType::GRAB_TYPE_OGM_OBJECT_LIST)
		.value("GRAB_TYPE_OGM_GEOMETRIES_LIST", GrabType::GRAB_TYPE_OGM_GEOMETRIES_LIST);
	...
	py::enum_<invz::RefPoint>(m, "RefPoint")
		...
		.value("Signal_Unfilled", invz::RefPoint::Signal_Unfilled);
	...
	py::enum_<invz::CalibrationStatus>(m, "CalibrationStatus")
		...
		.value("CalibStatus_PausedVehicleDynamics", invz::CalibrationStatus::CalibStatus_PausedVehicleDynamics)
	...
	py::class_<invz::SafetyMechnismsIndications>(m, "SafetyMechnismsIndications")
		.def(py::init<>())
		.def_readonly("sg1", &invz::SafetyMechnismsIndications::sg1)
		.def_readonly("sg2", &invz::SafetyMechnismsIndications::sg2)
		.def_readonly("tslr1", &invz::SafetyMechnismsIndications::tslr1)
		.def_readonly("tslr3", &invz::SafetyMechnismsIndications::tslr3)
		.def_readonly("tslr4", &invz::SafetyMechnismsIndications::tslr4);


	py::class_<invz::TlvValue_WatchDog>(m, "WatchDog")
		.def(py::init<>())
		.def_readonly("frame_number", &invz::TlvValue_WatchDog::frame_number)
		.def_readonly("internal_timestamp", &invz::TlvValue_WatchDog::internal_timestamp)
		.def_readonly("timestamp_sec", &invz::TlvValue_WatchDog::timestamp_sec)
		.def_readonly("timestamp_micro", &invz::TlvValue_WatchDog::timestamp_micro)
		.def_readonly("safetyMechanismIndications", &invz::TlvValue_WatchDog::safetyMechanismIndications)
		.def_readonly("lidarStateExtension", &invz::TlvValue_WatchDog::lidarStateExtension)
		.def_readonly("lidar_submode", &invz::TlvValue_WatchDog::lidar_submode)
		.def_readonly("lidar_mode", &invz::TlvValue_WatchDog::lidar_mode)
		.def_readonly("vbat", &invz::TlvValue_WatchDog::vbat);
	...
	py::class_<PY_DeviceInterface>(m, "DeviceInterface")
		...
		.def(py::init<const std::string, bool, int, std::string, uint32_t, bool, bool>(),
			"config_file_name"_a,
			"is_connect"_a = true,
			"login_level"_a = 0,
			"password"_a = "",
		...
		.def("deactivate_all_taps", &PY_DeviceInterface::DeactivateAllTaps,
			"Deactivate all taps. Equivalant to set_tap_activation_state(tap, false) for all taps.")
		...
		.def("cs_handshake", &PY_DeviceInterface::CSHandshake,
			"Perform cyber-security handshake with a connected device."
			"Connect must be called prior to calling this function.")
		.def("get_watch_dog", &PY_DeviceInterface::GetWatchDog,
			"Get a watch-dog message from the device")
		.def("get_Exchange_keys", &PY_DeviceInterface::GetExchangeKeys,
			"Get the device's exchange keys. "
			"Returns an array of 3 values - First: n, Second: e, Third: signature.")
		.def("encrypt_root_key", &PY_DeviceInterface::EncryptRootKey,
			"Encrypt a root key for cyber-security handshake with the device")
		.def("set_root_key", &PY_DeviceInterface::SetRootKey,
			"Set the device's root key");
	...
	py::enum_<invz::DataQualifier>(m, "DataQualifier")
		.value("k_normal", invz::DataQualifier::k_normal)
		.value("k_not_avaliable", invz::DataQualifier::k_not_avaliable)
		.value("k_reduce_in_coverage", invz::DataQualifier::k_reduce_in_coverage)
		.value("k_reduce_in_performance", invz::DataQualifier::k_reduce_in_performance)
		.value("k_reduce_in_coverage_and_performance", invz::DataQualifier::k_reduce_in_coverage_and_performance)
		.value("k_testMode", invz::DataQualifier::k_testMode)
		.value("k_invalid", invz::DataQualifier::k_invalid);

	py::enum_<invz::VehicleCoordinateSystem>(m, "VehicleCoordinateSystem")
		.value("k_rear_axle", invz::VehicleCoordinateSystem::k_rear_axle)
		.value("k_road_level", invz::VehicleCoordinateSystem::k_road_level)
		.value("k_front_axle", invz::VehicleCoordinateSystem::k_front_axle);

	py::enum_<invz::AsilLevel>(m, "AsilLevel")
		.value("k_QM", invz::AsilLevel::k_QM)
		.value("k_ASIL_A", invz::AsilLevel::k_ASIL_A)
		.value("k_ASIL_B", invz::AsilLevel::k_ASIL_B)
		.value("k_ASIL_C", invz::AsilLevel::k_ASIL_C)
		.value("k_ASIL_D", invz::AsilLevel::k_ASIL_D);

	py::enum_<invz::BlockageStatus>(m, "BlockageStatus")
		.value("k_FullBlockage", invz::BlockageStatus::k_FullBlockage)
		.value("k_PartialBlockageHighImpact", invz::BlockageStatus::k_PartialBlockageHighImpact)
		.value("k_PartialBlockageMediumImpact", invz::BlockageStatus::k_PartialBlockageMediumImpact)
		.value("k_PartialBlockageLowImpact", invz::BlockageStatus::k_PartialBlockageLowImpact)
		.value("Defect", invz::BlockageStatus::Defect)
		.value("None", invz::BlockageStatus::None);
	
	py::enum_<invz::ScanDataStartPosition>(m, "ScanDataStartPosition")
		.value("k_top_left", invz::ScanDataStartPosition::k_top_left)
		.value("k_top_right", invz::ScanDataStartPosition::k_top_right)
		.value("k_bottom_left", invz::ScanDataStartPosition::k_bottom_left)
		.value("k_bottom_right", invz::ScanDataStartPosition::k_bottom_right);

	py::enum_<invz::ScanDataDirection>(m, "ScanDataDirection")
		.value("k_column_first", invz::ScanDataDirection::k_column_first)
		.value("k_row_first", invz::ScanDataDirection::k_row_first);

	py::enum_<invz::Ambiguity>(m, "Ambiguity")
		.value("AMBIGUITY_UNDEFINED", invz::Ambiguity::AMBIGUITY_UNDEFINED)
		.value("AMBIGUITY_NONAMBIGUOUS", invz::Ambiguity::AMBIGUITY_NONAMBIGUOUS)
		.value("AMBIGUITY_AMBIGUOUS_180", invz::Ambiguity::AMBIGUITY_AMBIGUOUS_180)
		.value("AMBIGUITY_AMBIGUOUS_90", invz::Ambiguity::AMBIGUITY_AMBIGUOUS_90)
		.value("__LAST_REFPOINT_AMBIGUITY__", invz::Ambiguity::__LAST_REFPOINT_AMBIGUITY__);

	py::enum_<invz::MeasStatus>(m, "MeasStatus")
		.value("MS_UNDEFINED", invz::MeasStatus::MS_UNDEFINED)
		.value("MS_PREDICTION", invz::MeasStatus::MS_PREDICTION)
		.value("MS_MEASUREMENT", invz::MeasStatus::MS_MEASUREMENT)
		.value("__LAST_MEAS_STATUS_", invz::MeasStatus::__LAST_MEAS_STATUS_);

	py::enum_<invz::DimensionMeasuredStatus>(m, "DimensionMeasuredStatus")
		.value("DMS_UNKNOWN", invz::DimensionMeasuredStatus::DMS_UNKNOWN)
		.value("DMS_FULLY_MEASURED", invz::DimensionMeasuredStatus::DMS_FULLY_MEASURED)
		.value("DMS_PARTIALLY_MEASURED", invz::DimensionMeasuredStatus::DMS_PARTIALLY_MEASURED)
		.value("__LAST_DIMENSION_MEASURED_STATUS__", invz::DimensionMeasuredStatus::__LAST_DIMENSION_MEASURED_STATUS__);


	py::enum_<invz::OGMCalibrationStatus>(m, "OGMCalibrationStatus")
		.value("CALIB_UNKNOWN", invz::OGMCalibrationStatus::CALIB_UNKNOWN)
		.value("CALIB_NO_INITIAL_CALIBRATION", invz::OGMCalibrationStatus::CALIB_NO_INITIAL_CALIBRATION)
		.value("CALIB_NO_CLAMP15_CONFIRMATION", invz::OGMCalibrationStatus::CALIB_NO_CLAMP15_CONFIRMATION)
		.value("CALIB_OK_WITHIN_RANGE", invz::OGMCalibrationStatus::CALIB_OK_WITHIN_RANGE)
		.value("CALIB_MISALLIGNMENT_VALIDATION", invz::OGMCalibrationStatus::CALIB_MISALLIGNMENT_VALIDATION)
		.value("CALIB_NOT_OK_OUT_OF_RANGE", invz::OGMCalibrationStatus::CALIB_NOT_OK_OUT_OF_RANGE);

	py::enum_<invz::SensorStatusFlag>(m, "SensorStatusFlag")
		.value("SENSOR_STATUS_INIT", invz::SensorStatusFlag::SENSOR_STATUS_INIT)
		.value("SENSOR_STATUS_NOT_SET", invz::SensorStatusFlag::SENSOR_STATUS_NOT_SET)
		.value("SENSOR_STATUS_SET", invz::SensorStatusFlag::SENSOR_STATUS_SET)
		.value("__LAST_SENSOR_STATUS__", invz::SensorStatusFlag::__LAST_SENSOR_STATUS__);

	py::enum_<invz::Classes>(m, "Classes")
		.value("CLASS_UNKNOWN", invz::Classes::CLASS_UNKNOWN)
		.value("CLASS_OVERRIDE", invz::Classes::CLASS_OVERRIDE)
		.value("CLASS_UNDERRIDE", invz::Classes::CLASS_UNDERRIDE)
		.value("CLASS_FREESPACE", invz::Classes::CLASS_FREESPACE)
		.value("CLASS_DYNAMICOBJECT", invz::Classes::CLASS_DYNAMICOBJECT)
		.value("CLASS_SMALLOBJECT", invz::Classes::CLASS_SMALLOBJECT)
		.value("CLASS_BIGOBJECT", invz::Classes::CLASS_BIGOBJECT)
		.value("CLASS_ANIMAL", invz::Classes::CLASS_ANIMAL)
		.value("CLASS_ANIMALSMALL", invz::Classes::CLASS_ANIMALSMALL)
		.value("CLASS_CLUSTER", invz::Classes::CLASS_CLUSTER)
		.value("CLASS_TRAILER", invz::Classes::CLASS_TRAILER)
		.value("CLASS_AMBULANCE", invz::Classes::CLASS_AMBULANCE)
		.value("CLASS_FIREFIGHTING", invz::Classes::CLASS_FIREFIGHTING)
		.value("CLASS_POLICE", invz::Classes::CLASS_POLICE)
		.value("CLASS_AGRICULTURAL", invz::Classes::CLASS_AGRICULTURAL)
		.value("CLASS_TWOWHEELER", invz::Classes::CLASS_TWOWHEELER)
		.value("CLASS_BICYCLE", invz::Classes::CLASS_BICYCLE)
		.value("CLASS_MOTORBIKE", invz::Classes::CLASS_MOTORBIKE)
		.value("CLASS_FOURPLUSWHEELER", invz::Classes::CLASS_FOURPLUSWHEELER)
		.value("CLASS_PASSENGERCAR", invz::Classes::CLASS_PASSENGERCAR)
		.value("CLASS_VAN", invz::Classes::CLASS_VAN)
		.value("CLASS_TRUCK", invz::Classes::CLASS_TRUCK)
		.value("CLASS_BUS", invz::Classes::CLASS_BUS)
		.value("CLASS_VULNERABLEROADUSER", invz::Classes::CLASS_VULNERABLEROADUSER)
		.value("CLASS_PEDESTRIAN", invz::Classes::CLASS_PEDESTRIAN)
		.value("CLASS_PEDESTRIANGROUP", invz::Classes::CLASS_PEDESTRIANGROUP)
		.value("CLASS_STATICOBJECT", invz::Classes::CLASS_STATICOBJECT)
		.value("CLASS_ROADBOUNDARY", invz::Classes::CLASS_ROADBOUNDARY)
		.value("CLASS_GUARDRAIL", invz::Classes::CLASS_GUARDRAIL)
		.value("CLASS_CURB", invz::Classes::CLASS_CURB)
		.value("CLASS_POLE", invz::Classes::CLASS_POLE)
		.value("CLASS_FIELDOFVIEWBOUNDARY", invz::Classes::CLASS_FIELDOFVIEWBOUNDARY)
		.value("CLASS_ROADMARKING", invz::Classes::CLASS_ROADMARKING)
		.value("CLASS_ROADMARKING_START", invz::Classes::CLASS_ROADMARKING_START)
		.value("CLASS_ROADMARKING_END", invz::Classes::CLASS_ROADMARKING_END)
		.value("CLASS_INIT", invz::Classes::CLASS_INIT);

	py::enum_<invz::LongitudinalSide>(m, "LongitudinalSide")
		.value("LOS_UNKNOWN", invz::LongitudinalSide::LOS_UNKNOWN)
		.value("LOS_LEFT", invz::LongitudinalSide::LOS_LEFT)
		.value("LOS_RIGHT", invz::LongitudinalSide::LOS_RIGHT)
		.value("LOS_RADIUS", invz::LongitudinalSide::LOS_RADIUS)
		.value("__LAST_LONGITUDINAL_SIDE__", invz::LongitudinalSide::__LAST_LONGITUDINAL_SIDE__);

	py::enum_<invz::LateralSide>(m, "LateralSide")
		.value("LAS_UNKNOWN", invz::LateralSide::LAS_UNKNOWN)
		.value("LAS_FRONT", invz::LateralSide::LAS_FRONT)
		.value("LAS_REAR", invz::LateralSide::LAS_REAR)
		.value("LAS_RADIUS", invz::LateralSide::LAS_RADIUS)
		.value("__LAST_LATERAL_SIDE__", invz::LateralSide::__LAST_LATERAL_SIDE__);

	py::enum_<invz::ContourType>(m, "ContourType")
		.value("CONTOUR_TYPE_POINTS", invz::ContourType::CONTOUR_TYPE_POINTS)
		.value("CONTOUR_TYPE_POLYLINE", invz::ContourType::CONTOUR_TYPE_POLYLINE)
		.value("CONTOUR_TYPE_POLYGON", invz::ContourType::CONTOUR_TYPE_POLYGON);

	py::enum_<invz::GFlags>(m, "GFlags")
		.value("GF_INVALID", invz::GFlags::GF_INVALID)
		.value("GF_VALID", invz::GFlags::GF_VALID);

	py::enum_<invz::LineType>(m, "LineType")
		.value("LINE_TYPE_UNKNOWN", invz::LineType::LINE_TYPE_UNKNOWN)
		.value("LINE_TYPE_SOLID", invz::LineType::LINE_TYPE_SOLID)
		.value("LINE_TYPE_DASHED", invz::LineType::LINE_TYPE_DASHED)
		.value("LINE_TYPE_DOTTED", invz::LineType::LINE_TYPE_DOTTED)
		.value("__LAST_LINE_TYPE__", invz::LineType::__LAST_LINE_TYPE__);
