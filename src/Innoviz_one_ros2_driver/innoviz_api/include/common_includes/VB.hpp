...
#define VB_INVZBUF_PYBIND11_DTYPE_DECLERATIONS \
     ...
     * @description handle fields endianes, converts all fields to little endian.
     */
	inline void InnerPointLidar::handle_endianess() {
		// handle x endianness
		endianess_switch(&x, sizeof(InnerPointLidar::x));

		// handle y endianness
		endianess_switch(&y, sizeof(InnerPointLidar::y));
	...
	inline  void ObjectPodLidar::handle_endianess(){
		...
		// handle inner_point endianness
		inner_point.handle_endianess();
