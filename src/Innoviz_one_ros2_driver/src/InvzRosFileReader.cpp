...
IReader* FileReaderInitByParams(rclcpp::Node& rosNode)
{
    // declare necessary parameters
    string filePath = getRosParam<std::string>("file_path",rosNode);
    string invzLogPath = getRosParam<std::string>("invz_log_path",rosNode);
    string configFilePath = getRosParam<std::string>("config_file_path",rosNode);
    int invzLogLevel = getRosParam<int>("invz_log_level",rosNode);
 ...
 */
InvzRosFileReader::InvzRosFileReader(	// current node handle
    rclcpp::Node& rosNode,
	rclcpp::Logger& logger) :
    InvzRosIReader({ FileReaderInitByParams(rosNode), closeFileReader }, 0, false,rosNode,logger)
{
